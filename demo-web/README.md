...
The goal is a server-rendered frontend with React islands for interactive UI, running as a Node process.
...
| Runtime        | Node (`@astrojs/node` standalone)                         |
...
```sh
pnpm build
pnpm start    # node ./dist/server/entry.mjs  (HOST / PORT env)
```
...
This is a Node process (`@astrojs/node` standalone). `pnpm build` writes `dist/server/entry.mjs`; `pnpm start` runs it.
...
Set `PUBLIC_API_URL` at build time (Astro inlines `PUBLIC_` vars). Set `HOST` / `PORT` at runtime.
